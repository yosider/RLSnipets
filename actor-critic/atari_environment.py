...
class AtariEnvironment(object):
    ...
    of size sequence_length from which environment state
    is constructed.
    """
    def __init__(self, gym_env, resized_width, resized_height, sequence_length):
        self.env = gym_env
    ...
    def step(self, action_index):
        ...
        Builds current state (concatenation of sequence_length-1 previous frames and current one).
        ...
        s_t1 = np.empty((self.sequence_length, self.resized_height, self.resized_width))
        s_t1[:self.sequence_length-1, ...] = previous_frames
        s_t1[self.sequence_length-1] = x_t1
